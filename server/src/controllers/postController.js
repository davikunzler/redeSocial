...
exports.toggleFavorite = async (req, res) => {
  ...
  try {
    ...
    if (existingFavorite.length > 0) {
      ...
      res
        .status(200)
        .json({ message: "Favorito removido com sucesso.", favorited: false });
    } else {
      ...
      res
        .status(201)
        .json({ message: "Post adicionado aos favoritos.", favorited: true });
    }
  } catch (error) {
...
exports.searchPosts = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Erro interno do servidor ao buscar/pesquisar posts." });
  }
};
